...
function character:init()
    ...
    self:setDarkTransitionActor("asriel_dark_transition")
    ...
    self.color = {0.6, 0.35, 1}
    -- Damage color (for the number when attacking enemies) (defaults to the main color)
    self.dmg_color = {0.6, 0.35, 1}
    -- Attack bar color (for the target bar used in attack mode) (defaults to the main color)
    self.attack_bar_color = {0.6, 0.35, 1}
    -- Attack box color (for the attack area in attack mode) (defaults to darkened main color)
    self.attack_box_color = {0.6, 0.35, 1}
    -- X-Action color (for the color of X-Action menu items) (defaults to the main color)
    self.xact_color = {0.6, 0.35, 1}
    ...
    self.attack_sprite = "effects/attack/cut_a"
...
function character:onPowerSelect(menu)
    if Utils.random() < ((Game.chapter == 1) and 0.02 or 0.04) then
        menu.asriel_dog = true
    else
        menu.asriel_dog = false
    end
end

function character:drawPowerStat(index, x, y, menu)
    if index == 1 and menu.asriel_dog then
