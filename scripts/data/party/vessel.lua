...
function character:init()
    ...
    self.stronger_absent = {"asriel","dess","vessel"}
    ...
    self.attack_bar_color = {0.5, 0.5, 0.5}
    -- Attack box color (for the attack area in attack mode) (defaults to darkened main color)
    self.attack_box_color = {0.5, 0.5, 0.5}
    -- X-Action color (for the color of X-Action menu items) (defaults to the main color)
    self.xact_color = {0.5, 0.5, 0.5}

    -- Head icon in the equip / power menu
    if ralsei_style == 1 then
        self.menu_icon = "party/vessel/head_ch1"
    else
    ...
    self.attack_sprite = "effects/attack/slap_v"
...
function character:onPowerSelect(menu)
    if Utils.random() <= 0.03 then
        menu.vessel_dog = true
    else
        menu.vessel_dog = false
    end
end
...
function character:drawPowerStat(index, x, y, menu)
    if index == 1 then
        if Game.chapter == 1 then
            -- Chapter 1 Vessel "Kindness" stat (doggable)
            if not menu.vessel_dog then
            ...
            -- Chapter 2 Vessel "Sweetness" stat (non-doggable)
